...
export function App() {
  ...
  useEffect(() => {
    if (urlTeam && urlTeam !== activeTeamKey) {
      writeStoredTeamKey(urlTeam);
      setActiveTeamKey(urlTeam);
      setExpandedTeamKey(urlTeam);
    }
  ...
  const paletteActions = useMemo<PaletteAction[]>(() => {
    const actions: PaletteAction[] = [
      ...
      },
      {
        ...
        description: 'Review completed agent runs awaiting human acceptance',
        group: 'Navigation',
        shortcut: 'G N',
                ...
                {shellTeams.map((team) => {
                  ...
                  return (
                    <div key={team.id} className="app-shell__team-group">
                      <NavLink
                        to={`/?team=${encodeURIComponent(team.key)}`}
                      ...
                      >
                        <span
                          className={`app-shell__team-caret${isExpanded ? ' app-shell__team-caret--expanded' : ''}`}
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setExpandedTeamKey(isExpanded ? null : team.key);
                          }}
                        >
                          <IcoChevD size={12} />
                        </span>
                      ...
                      {isExpanded ? (
                        <div className="app-shell__team-subnav">
                          ...
                          <NavLink
                            to={`/backlog?team=${encodeURIComponent(team.key)}`}
                          ...
                          <NavLink
                            to={`/cycles?team=${encodeURIComponent(team.key)}`}
